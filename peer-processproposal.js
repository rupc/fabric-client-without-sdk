...
let nonce;
async function calculateTransactionId() {
	nonce = crypto.randomBytes(24);
    const creator_bytes = Buffer.from("creator");
    const trans_bytes = Buffer.concat([nonce, creator_bytes]);
    const trans_hash = crypto.createHash('sha256').update(trans_bytes).digest('hex');
...
async function buildPayload() {
    ...
    const txid = await calculateTransactionId();
